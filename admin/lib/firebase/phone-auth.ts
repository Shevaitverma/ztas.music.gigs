...
export function initRecaptcha(buttonId: string): RecaptchaInitResult {
  ...
  if (!status.isReady) {
    return {
      success: false,
      verifier: null,
      error: status.error || 'Firebase Auth is not initialized.',
    }
  }
  const auth = getFirebaseAuth()
  if (!auth) {
    return { success: false, verifier: null, error: 'Firebase Auth could not be initialized.' }
  }
...
export async function sendOtp(phoneNumber: string): Promise<boolean> {
  ...
  try {
    ...
    if (authError.code === 'auth/invalid-phone-number') throw new Error('Invalid phone number format')
...
export async function verifyOtp(otp: string): Promise<PhoneSignInResult> {
  ...
  try {
    ...
    return {
      idToken,
      user: { uid: result.user.uid, phoneNumber: result.user.phoneNumber },
    }
  } catch (error) {
    const authError = error as { code?: string; message?: string }
